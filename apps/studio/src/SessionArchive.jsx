...
const harnessName=h=>h==='claude'?'Claude Code':'Codex';
const readableSource=source=>source?{...source,...source.sources?.find(s=>s.available)}:null;
const readError=e=>e.name==='TimeoutError'?'读取时间较长，稍后会自动重试。':e.message||'记录暂时无法读取，请稍后刷新。';

function PublicSessionReader({source, projects, onSource, draft, setDraft}) {
  ...
  useEffect(()=>{
    ...
    let timer;position.current={generation:'',cursor:null};setEvents([]);setGeneration('');setOlder(null);setOlderBusy(false);setReady(false);setError('');setBoundary('');
    async function poll(){
      try{
        ...
        if(value.reset)setBoundary('原日志已更换或截断，已读取当前文件，没有拼接旧文件内容。');
  ...
  return <div className="session-detail archive-reader">
    <header className="session-detail-head"><span className="eyebrow">{harnessName(source.harness)} · 公开记录</span><h2>{source.title}</h2><p>{projectNames.length?projectNames.join(' / '):'尚未归类 · 查询无需先建项目'}</p><details><summary>任务编号与工作目录</summary><code>{source.session_id}</code><p className="agent-path">{source.cwd || '目录未记录'}</p>{source.sources?.length>1&&<><p>此任务保留 {source.sources.length} 条来源；项目名称只说明已登记的来源，不据此更改归属。</p><label className="archive-harness">阅读哪份记录<select value={source.source_id} onChange={e=>onSource({...source,...source.sources.find(s=>s.source_id===e.target.value)})}>{source.sources.map((s,i)=><option key={s.source_id} value={s.source_id}>记录 {i+1} · {projects.find(p=>p.project_id===s.project_id)?.display_name||s.project_id||'未归类'} · {s.available?'可读取':'暂不可读'}</option>)}</select></label></>}</details></header>
    ...
    {source.harness==='codex'&&source.session_id?<SessionComposer key={source.session_id} session={source} draft={draft} setDraft={setDraft}/>:<div className="archive-continue"><span>普通 Claude 终端只读监看；在原终端恢复后，Skill 会读取项目与其他任务的进展。</span><button className="secondary-button" onClick={async()=>{try{await navigator.clipboard.writeText('claude --resume '+JSON.stringify(source.session_id));setCopied(true);}catch{setError('剪贴板不可用，可在原终端使用 claude --resume '+source.session_id);}}}>{copied?'继续命令已复制':'复制继续命令'}</button></div>}
  </div>;
}

export function SessionArchive({projects=[],onBack,getDraft,changeDraft}) {
  const [search,setSearch]=useState(''),[query,setQuery]=useState(''),[harness,setHarness]=useState('');
  const [offset,setOffset]=useState(0),[catalog,setCatalog]=useState(null),[selected,setSelected]=useState(null);
  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      try{
        ...
        setCatalog(value);setError('');setSelected(old=>old||readableSource(value.sessions.find(s=>s.available)||value.sessions[0]));
  ...
  return <div className="page-stack sessions-page archive-page">
    <div className="page-header"><div><span className="eyebrow">会话 · 历史查询</span><h1>更早的任务，也能接着读</h1><p>按标题、任务编号或工作目录查找 Codex 与 Claude Code；消息按需读取，项目资料不会因此改变。</p></div><button className="secondary-button" onClick={onBack}>返回近期监看</button></div>
    <div className="session-workspace"><aside className="session-browser archive-browser"><div className="session-browser-head"><strong>{catalog?`已发现 ${catalog.total} 条匹配任务`:'正在读取任务目录…'}</strong><button className="text-action" onClick={()=>setRefresh(n=>n+1)} disabled={loading}>刷新目录</button></div>
      <form className="archive-search" onSubmit={e=>{e.preventDefault();filter('query',search.trim());}}><label className="search-field"><input aria-label="查找历史任务" value={search} onChange={e=>setSearch(e.target.value)} placeholder="标题、任务编号、工作目录"/></label><button className="secondary-button" type="submit">查找</button></form>
      <label className="archive-harness">来自哪个应用<select value={harness} onChange={e=>filter('harness',e.target.value)}><option value="">Codex 与 Claude Code</option><option value="codex">Codex</option><option value="claude">Claude Code</option></select></label>
      {error&&<p role="status" className="agent-notice">{error} {catalog?'已保留本页成功读取的目录。':'可以点击刷新目录重试。'}</p>}
      <div className="session-browser-list" aria-busy={loading}>{catalog?.sessions.map(s=><button className={'session-card '+(selected?.session_id===s.session_id&&selected?.harness===s.harness?'selected':'')} key={s.harness+':'+(s.session_id||s.source_id)} onClick={()=>choose(s)}><small>{harnessName(s.harness)} · {s.available?'可读取记录':'日志暂不可读'}</small><h3>{s.title}</h3><p className="session-title-source">{s.modified_at?new Date(s.modified_at).toLocaleString('zh-CN'):'更新时间未记录'}</p></button>)}{!catalog?.sessions.length&&<p className="muted">{loading?'正在查找历史任务…':'没有匹配任务，换一个关键词或应用试试。'}</p>}</div>
      ...
      <details className="archive-coverage"><summary>当前目录包含哪些记录？</summary><p>Codex：已经发现并登记的来源。Claude：本机已配置目录中的可发现来源。文件更新时间不代表任务仍在执行。</p>{catalog?.coverage?.claude?.has_more_sources&&<p>Claude 发现目录当前有读取上限，尚不包含更早的全部文件。</p>}{catalog?.warnings?.map((w,i)=><p key={i}>{w}</p>)}</details>
    </aside><section ref={reader} className="session-detail-pane" aria-label="历史任务的对话">{selected?<PublicSessionReader key={selected.source_id} source={selected} projects={projects} onSource={setSelected} draft={getDraft(selected)} setDraft={value=>changeDraft(selected,value)}/>:<div className="page-section"><h2>选择一条历史任务</h2><p>左侧可搜索、翻页或切换应用。打开后先显示最新公开消息。</p></div>}</section></div>
  </div>;
}
