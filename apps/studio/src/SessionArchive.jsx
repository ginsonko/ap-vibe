...
export function PublicSessionReader({source, projects, onSource, draft, setDraft}) {
  ...
  useEffect(()=>{
    ...
    async function poll(){
      try{
        ...
        setMetadata(Object.fromEntries(['title','title_source','model','model_source','observed_models','state','managed','run_id','task_id'].filter(key=>key in value).map(key=>[key,value[key]])));
  ...
  return <div className="session-detail archive-reader">
    ...
    {source.managed?<div className="archive-continue"><span>工作室托管任务：在 Agent 工作室的任务池查看依赖、接手和验收，公开输出会持续显示在这里。</span></div>:source.harness==='codex'&&source.session_id?<SessionComposer key={source.session_id} session={source} draft={draft} setDraft={setDraft}/>:<div className="archive-continue"><span>普通 Claude 终端只读监看；在原终端恢复后，Skill 会读取项目与其他任务的进展。</span><button className="secondary-button" onClick={async()=>{try{await navigator.clipboard.writeText('claude --resume '+JSON.stringify(source.session_id));setCopied(true);}catch{setError('剪贴板不可用，可在原终端使用 claude --resume '+source.session_id);}}}>{copied?'继续命令已复制':'复制继续命令'}</button></div>}
  </div>;
}
...
export function SessionArchive({projects=[],onBack,getDraft,changeDraft,mode='history',requestedSession=null,onSelectionChange}) {
  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      try{
        ...
        setCatalog(value);setError('');setSelected(old=>{
  ...
  return <div className="page-stack sessions-page archive-page">
    <div className="page-header"><div><span className="eyebrow">会话{mode==='history'?' · 历史查询':''}</span><h1>{mode==='history'?'更早的任务，也能接着读':'所有任务，在这里接着看'}</h1><p>Codex 与 Claude Code 按最近记录更新排列；选择任务就能看最新消息，也可以向上翻阅历史。</p></div>{onBack&&<button className="secondary-button" onClick={onBack}>返回近期监看</button>}</div>
    <div className="session-workspace"><aside className="session-browser archive-browser"><div className="session-browser-head"><strong>{catalog?`已发现 ${catalog.total} 条匹配任务`:'正在读取任务目录…'}</strong><button className="text-action" onClick={()=>setRefresh(n=>n+1)} disabled={loading}>刷新目录</button></div>
      ...
      <label className="archive-harness">来自哪个应用<select aria-label="会话应用筛选" value={harness} onChange={e=>filter('harness',e.target.value)}><option value="">全部 · Codex 与 Claude Code</option><option value="codex">仅 Codex</option><option value="claude">仅 Claude Code</option></select></label>
      {error&&<p role="status" className="agent-notice">{error} {catalog?'已保留本页成功读取的目录。':'可以点击刷新目录重试。'}</p>}
      <div className="session-browser-list" aria-busy={loading}>{catalog?.sessions.map(s=><button className={'session-card '+(sameSession(selected,s)?'selected':'')} key={s.harness+':'+(s.session_id||s.source_id)} onClick={()=>choose(s)}><small>{harnessName(s.harness)} · {s.managed?'工作室托管 · '+stateName(s.state):s.available?'可读取记录':'日志暂不可读'}</small><h3>{s.title}</h3>{s.harness==='claude'&&<p className="session-model">{s.model||'模型尚未记录'}</p>}<p className="session-title-source">{s.modified_at?new Date(s.modified_at).toLocaleString('zh-CN'):'更新时间未记录'}</p></button>)}{!catalog?.sessions.length&&<p className="muted">{loading?'正在查找任务…':'没有匹配任务，换一个关键词或应用试试。'}</p>}</div>
      ...
      <details className="archive-coverage"><summary>当前目录包含哪些记录？</summary><p>包含 Codex、普通 Claude Code 和工作室托管任务。托管任务显示已保存的真实执行状态；普通日志的更新时间不代表仍在执行。</p>{catalog?.coverage?.claude?.has_more_sources&&<p>Claude 发现目录当前有读取上限，尚不包含更早的全部文件。</p>}{catalog?.warnings?.map((w,i)=><p key={i}>{w}</p>)}</details>
