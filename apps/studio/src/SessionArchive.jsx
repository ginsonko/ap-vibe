...
const harnessName=h=>({codex:'Codex',claude:'Claude Code',opencode:'OpenCode',openclaw:'OpenClaw',pi:'PI CLI','pi-desktop':'PI Desktop',mimocode:'MiMo Code',zcode:'ZCode','ga-admin':'GenericAgent Admin',hermes:'Hermes',dsh:'DSH Desktop',grok:'Grok Desktop / CLI',workbuddy:'WorkBuddy / CodeBuddy CLI'}[h]||h);
...
export function PublicSessionReader({source, projects, onSource, draft, setDraft}) {
  ...
  return <div className="session-detail archive-reader">
    ...
    {source.managed?<div className="archive-continue"><span>工作室托管任务：在 Agent 工作室的任务池查看依赖、接手和验收，公开输出会持续显示在这里。</span></div>:(source.harness==='codex'||source.harness==='grok'&&source.native_surface==='desktop')&&source.session_id?<SessionComposer key={source.session_id} session={source} draft={draft} setDraft={setDraft}/>:<div className="archive-continue"><span>普通 {harnessName(source.harness)} 会话显示公开记录；在原应用继续后，可通过 AP-Vibe 读取项目、其它任务进展和收件箱。</span>{source.harness==='claude'&&<button className="secondary-button" onClick={async()=>{try{await navigator.clipboard.writeText('claude --resume '+JSON.stringify(source.session_id));setCopied(true);}catch{setError('剪贴板不可用，可在原终端使用 claude --resume '+source.session_id);}}}>{copied?'继续命令已复制':'复制继续命令'}</button>}</div>}
  </div>;
}
