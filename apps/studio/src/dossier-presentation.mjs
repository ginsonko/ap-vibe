...
export function projectDescription(project) {
  ...
  const summary = project.description?.trim() || '项目简介尚未填写，可以选择整理执行端从真实来源补充。';
...
export function curationError(value) {
  ...
  if (/overload|Too many pending|429|503/i.test(detail)) return '整理模型上游繁忙。已保留草稿，稍后可继续未完成项目。';
  if (/timeout|SSE|timed.out/i.test(detail)) return '整理模型本次等待超时。已保留草稿，可继续未完成项目。';
  if (/disconnect|stream closed/i.test(detail)) return '整理模型连接中断。已保留草稿，可继续未完成项目。';
  ...
  if (/cli_missing|executor_unavailable/i.test(detail)) return '所选整理执行端不可用。请选择已安装的客户端、已激活伙伴，或交给当前软件通过 MCP / Skill 整理。';
