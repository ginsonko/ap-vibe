...
export function AgentBudget({agent, onClose, onSaved}) {
  ...
  return <div className="agent-dialog-backdrop"><section ref={dialog} className="agent-dialog budget-dialog" role="dialog" aria-modal="true" aria-label="用量与投喂">
    ...
    <details><summary>参考价格与费用估算（可交给 Codex 填写）</summary>
      {agent.price_reference && <div className="agent-reference-price"><p>银子 API {agent.price_reference.channel==='ccmax'?'CC Max 专用渠道':'常规渠道'}历史样本：每亿输入综合成本 ¥{agent.price_reference.effective_per_100m_input_cny}，缓存率 {(agent.price_reference.cache_rate*100).toFixed(1)}%。按总 token 折合约 ¥{agent.price_reference.blended_per_million_total_cny} / 百万。</p><p>已包含样本缓存收益，不能再统一打折；这不是服务商原始单价。当前设置只在保存后改变，旧账保留。</p><button type="button" disabled={busy} onClick={()=>{setDraft(old=>({...old,prices:agent.price_reference.prices,price_basis:'blended',price_source:agent.price_reference.price_source,currency:'CNY'}));setNotice('参考值已填入；核对后保存用量设置。原上限保持。');}}>填入此参考价</button></div>}
