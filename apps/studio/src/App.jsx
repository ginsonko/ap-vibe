...
function HelpPage({ onNavigate, onCopy }) {
  const helpCards = [
    ...
    { icon: Database, title: "项目与记忆：项目说明书", when: "你想知道项目做什么、为什么这样设计、哪些完成了，或有哪些薄弱环节时。", steps: ["切换到正确项目，先看简介与用户目标。", "按目录打开需要的章节；资料由已接入的软件在工作中整理，也可在整理页选择执行端。", "查看十维雷达图，点击每项了解评分理由与改进建议。"], example: "请使用 AP-Vibe 读取这个项目的资料目录，核对简介、红线与待办；完成工作后更新相关章节，评分必须附理由和证据。" },
...
function Topbar({ page, project, health, refreshing, onRefresh, onSync, syncBusy }) {
  const current = page === "organization" ? { label: "档案整理", description: "项目与记忆 / 跨客户端整理进度" } : NAV_ITEMS.find((item) => item.id === page) || NAV_ITEMS[0];
...
export function App() {
  ...
  else if (page === "help") content = <HelpPage onNavigate={navigate} onCopy={async value => { try { await navigator.clipboard.writeText(value); showNotice("good", "示例已复制", "可以粘贴给已接入的软件使用。"); } catch { showNotice("bad", "复制未成功", "请选中示例文字手动复制。"); } }} />;
