...
经济分工默认开启于已启用的协作：调用 ap_vibe_agent_recommendations，tags写本项可复用类别（routine_code/frontend/writing/research/reasoning/planning/review；困难或关键可靠性加critical/complex/hard）。读取排序原因与样本覆盖，结合用户职责、连接和忙碌选择。Astra/Codex主要做简短关键规划及最终确认，CC Max用于攻坚；明确、耗时或批量工作优先合适的经济伙伴，普通检查不默认升级高价。几分钟的小事直接做，不为省钱增加协调成本。用户明确指定优先；未知能力仍可选择。简历是可编辑初期偏好，战绩按同类已验收逻辑任务平滑加权，重试不多算，网络故障不当质量失败。实际效果和总成本都要保留证据，不承诺每次分工都省钱。详情见[经济分工参考](economical-routing.md)。
